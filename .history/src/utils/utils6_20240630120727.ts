...
export const criarCampos6 = (times: Time[], jogadoresEsperando: Jogador[]): CampoType[] => [
  { id: 1, times: times.slice(0, 2), jogadoresEsperando },
];
...
export const trocarJogadores6 = (jogos: CampoType[][], campoId: number, jogadoresEsperando: Jogador[]): CampoType[][] => {
    ...
    jogo.map(campo => {
      if (campo.id === campoId) {
        ...
        if (novosTimes.length === 2) {
          ...
          if (perdedores) {
            const novosJogadoresEsperando = [...jogadoresEsperando, ...perdedores.jogadores];
            const jogadoresQueEntram = novosJogadoresEsperando.splice(0, 2);
            novosTimes[novosTimes.indexOf(perdedores)].jogadores = jogadoresQueEntram;
            return { ...campo, times: novosTimes, jogadoresEsperando: novosJogadoresEsperando };
          }
        }
      }
      return campo;
