...
const CalculadoraApp: React.FC = () => {
  ...
  const [segundoJogo, setSegundoJogo] = useState<Campo[]>([
    { id: 1, times: [] },
    { id: 2, times: [] },
    { id: 3, times: [] },
  ]);
  ...
  const handleResultadoChange = (campoId: number, timeIndex: number, novoResultado: number, jogo: string) => {
    if (jogo === 'primeiro') {
      ...
      setSegundoJogo(segundoJogo.map(campo =>
        campo.id === campoId
          ? {
              ...campo,
              times: campo.times.map((time, index) =>
  ...
  return (
    <div className="calculadora-container">
      ...
      <div className="main-content">
                  ...
                  <li key={idx} className={getTeamClass(campo, idx)}>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleResultadoChange(campo.id, idx, parseInt(e.target.value), 'primeiro')}
        ...
        {segundoJogo[0].times.length > 0 && (
          <div className="fields-container">
            <h2>Resultados Jogo Dois</h2>
            {segundoJogo.map(campo => (
              <div key={campo.id} className="field">
                <h2>Campo {campo.id}</h2>
                <ul>
                  {campo.times.map((time, idx) => (
                    <li key={idx} className={getTeamClass(campo, idx)}>
                      <div>
                        ...
                        <input
                          type="number"
                          value={time.resultado}
                          onChange={(e) => handleResultadoChange(campo.id, idx, parseInt(e.target.value), 'segundo')}
                          className="resultado-input"
                        />
                      </div>
                    </li>
