...
const criarTimes = (jogadores: Jogador[]): Time[] => {
  ...
  for (let i = 0; i < jogadoresEmbaralhados.length; i += 2) {
    times.push({
      jogadores: jogadoresEmbaralhados.slice(i, i + 2),
      resultado: 0,
    });
  }
  return times;
      ...
      jogo.forEach((campo) => {
        campo.times.forEach((time) => {
          if (time.jogadores.some((j) => j.id === jogador.id)) {
            ...
            if (
              time.resultado >
              campo.times[1 - campo.times.indexOf(time)].resultado
            ) {
              vitorias += 1;
            }
          }
...
const CalculadoraApp: React.FC = () => {
  ...
  const iniciarProximoJogoHandler = () => {
    ...
    if (campo1 && campo2 && campo3) {
      ...
      const vencedoresCampo1 =
        time1Campo1.resultado > time2Campo1.resultado
          ? time1Campo1.jogadores
          : time2Campo1.jogadores;
      const perdedoresCampo1 =
        time1Campo1.resultado > time2Campo1.resultado
          ? time2Campo1.jogadores
          : time1Campo1.jogadores;

      const vencedoresCampo2 =
        time1Campo2.resultado > time2Campo2.resultado
          ? time1Campo2.jogadores
          : time2Campo2.jogadores;
      const perdedoresCampo2 =
        time1Campo2.resultado > time2Campo2.resultado
          ? time2Campo2.jogadores
          : time1Campo2.jogadores;

      const vencedoresCampo3 =
        time1Campo3.resultado > time2Campo3.resultado
          ? time1Campo3.jogadores
          : time2Campo3.jogadores;
      const perdedoresCampo3 =
        time1Campo3.resultado > time2Campo3.resultado
          ? time2Campo3.jogadores
          : time1Campo3.jogadores;
  ...
  const todosResultadosInseridos =
    jogos.length === 5 &&
    jogos.every((jogo) =>
      jogo.every((campo) =>
        campo.times.every((time) => typeof time.resultado === "number")
      )
    );
      ...
      <div className="main-content">
        ...
        <div className="fields-container">
          ...
          {jogos.length > 0 && jogos.length < 5 && (
            <button
              onClick={iniciarProximoJogoHandler}
        ...
        <div id="ranking-resultados">
          <div className="export-button-container">
            {todosResultadosInseridos && (
              <>
                <ExportButton
                  jogadores={jogadoresClassificados}
                  jogos={jogos}
                />
