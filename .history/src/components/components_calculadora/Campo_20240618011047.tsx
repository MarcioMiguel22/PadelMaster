...
interface CampoProps {
  ...
  trocarJogadores: (campoId: number, jogadoresAtuais: Jogador[]) => void; // Adicionando a prop trocarJogadores
}
...
const Campo: React.FC<CampoProps> = ({ campo, jogoIndex, handleResultadoChange, handleNomeEdit, getTeamClass, trocarJogadores }) => (
  <div className="field">
    ...
    {jogoIndex > 0 && (
      <button onClick={() => trocarJogadores(campo.id, campo.times.flatMap(time => time.jogadores))} className="trocar-jogadores-button">
        Trocar Jogadores
      </button>
    )}
  </div>
